 ...
 *     Sebastian Zarnekow <Sebastian.Zarnekow@gmail.com> - Bug 519776
 *******************************************************************************/
...
public class Synchronizer implements ISynchronizer {
	...
	public void setSyncInfo(QualifiedName partner, IResource resource, byte[] info) throws CoreException {
		...
		Assert.isLegal(resource != null);
		/*
		 * We use a dedicated sync rule for this operation and not the resoure
		 * itself directly. This helps to avoid cancelled auto builds when team provider
		 * trigger a #setSyncInfo from within a build, e.g. when a builder generates code
		 * into directories, that are monitored by the team provider.
		 */
		ISchedulingRule syncRule = workspace.getRuleFactory().syncInfoRule(resource);
		try {
			workspace.prepareOperation(syncRule, null);
			...
			workspace.endOperation(syncRule, false, null);
		}
	}
